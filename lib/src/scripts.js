...
exports.loadScripts = function () {
    ...
    var paths = pluginPaths.map < { plugin: string | null, path: string } > (function (p) {
        return {
        ...
        if (fs.existsSync(p.path)) {
            for (var _a = 0, _b = fs.readdirSync(p.path); _a < _b.length; _a++) {
                ...
                if (path.extname(filename) === '.js') {
                    var name_1 = (p.plugin ? p.plugin + "/" : '') + path.basename(filename).replace('.js', '');
                    scripts.push(model_1.createNappJSScript(name_1, path.join(p.path, filename)));
                }
            }
        }
    }
